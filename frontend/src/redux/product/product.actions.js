...
export const getProducts = (page,sorting,gender,category,brand,color,minVal,maxVal,searchByValue,id)=> async(dispatch)=> {
    dispatch(setLoading({type:LOADING_PRODUCTS}));
    try {
        let res;
        res = await fetch(`${process.env.REACT_APP_BASEURL}products?page=${page}`);
        if(searchByValue){
            res = await fetch(`${process.env.REACT_APP_BASEURL}products?find=${searchByValue}&page=${page}`);
        }else{
            if(id){
                res = await fetch(`${process.env.REACT_APP_BASEURL}products?gender=${id}&page=${page}`);
            }
            if(sorting){
